...
이 Part의 목표는 복잡한 UI를 "상태와 계약"으로 읽는 능력을 기르는 것이다. AI에게 컴포넌트를 바로 만들게 하기 전에, 먼저 상태 지도와 타입 계약을 만든다.

실습 과제:

- 실제 기능 하나를 고른다. 검색, 결제, 알림 설정, 초대 플로처럼 상태가 둘 이상 있는 기능이 좋다.
- 그 기능의 상태를 지역 상태, 공유 UI 상태, 서버 상태, URL 상태, 파생 상태로 분류한다.
- API 응답 타입과 UI view model 타입을 분리해서 적는다.
- 가능한 UI 상태를 boolean 조합이 아니라 union type 형태로 표현한다.
- AI에게 이 상태 지도와 타입 계약을 주고 구현 초안을 요청한다.

회고 질문:

- 상태를 분류하기 전과 후에 AI 지시문의 정확성이 달라졌는가?
- UI가 API 응답 구조에 과하게 끌려가고 있지는 않은가?
- optional 필드가 "없을 수도 있음" 이상의 의미를 숨기고 있지는 않은가?
- 모순 가능한 상태 조합을 타입으로 막을 수 있었는가?

완료 체크:

- 상태 분류표를 작성했다.
- API 타입과 UI 타입을 분리했다.
- union type 또는 상태 머신으로 주요 UI 상태를 표현했다.
- AI가 만든 코드에서 계약을 어긴 부분을 하나 이상 리뷰했다.
